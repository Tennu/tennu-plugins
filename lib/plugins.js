...
const PluginSystem = function (context) {
    ...
    /*
    enum PluginInstallableValidation {
        canInstall: Boolean,
    ...
    If canInstall is `true`, no other fields will be set.

    If canInstall is `false`, `failureReason` and `reason` will be set. `help` may be set.
    */

    // isInstallable(pluginFactory: any*)) -> IsInstallableValidation
    // * probably a PluginFactory
    function isInstallable(pluginFactory) {
        return isInstallableWith(pluginFactory, []);
    }

    // isInstallableWith(pluginFactory: any*, assumedPlugins: [PluginFactory]) -> IsInstallableValidation
    // * probably a PluginFactory
    ...
    function isInstallableWith(pluginFactory, assumedPlugins) {
    ...
    function install(pluginFactory) {
        const validation = isInstallable(pluginFactory);
    ...
    return {
        ...
        isInstallableWith: isInstallableWith,
