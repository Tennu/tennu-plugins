...
const PluginSystem = function (context) {
    ...
    function addInstanceHook (hook, fn) {
        ...
        return setInstanceHookResult.mapFail(function (failure) {
            return {
                ...
                message: `Tried to set instance hook '${hook}', but a plugin already has that instance hook.`,
                hook
            };
        });
    }
    ...
    function addStaticHook (hook, fn) {
        ...
        return setStaticHookResult.mapFail(function (failure) {
            return {
                ...
                message: `Tried to set static hook '${hook}', but a plugin already has that static hook.`,
                hook
            };
        });
    }
    ...
    If canInitialize is `false`, `failureReason` and `reason` will be set. `help` may be set.
    */

    // isInitializable(pluginFactory: any*)) -> IsInitializableValidation
    // * probably a PluginFactory
    function isInitializable(pluginFactory) {
        return isInitializableWith(pluginFactory, []);
    }

    // isInitializableWith(pluginFactory: any*, assumedPlugins: [PluginFactory]) -> IsInitializableValidation
    // * probably a PluginFactory
    // Says if a plugin could be initialized if we also assumed the Plugins in
    // the second list were also initialized.
    // WHY(Havvy): This function is used by `use` in loader.js because we want
    //             to know which plugins cannot be initialized due to missing
    //             dependencies that aren't in the list passed to `use`.
    function isInitializableWith(pluginFactory, assumedPlugins) {
        if (typeof pluginFactory !== "object" || pluginFactory === null) {
            const pluginType = typeof pluginFactory === "object" ? "null" : typeof pluginFactory;

            return {
                canInitialize: false,
                pluginFactory,
                failureReason: failures.validation.NotAnObject,
        ...
        if (typeof pluginFactory.name !== "string") {
            return {
                ...
                help: `PluginFactory 'name' property is a ${typeof pluginFactory.name} instead.`
            }
        }
        ...
        if (plugins.has(pluginFactory.name) || assumedPlugins.some(function (assumedPluginFactory) {
            ...
            return {
                ...
                reason: `A plugin with the name '${pluginFactory.name}' has already been installed.`
            }
        }

        if (pluginFactory.role) {
            if (typeof pluginFactory.role !== "string") {
                return {
                    canInitialize: false,
                    pluginFactory,
                    failureReason: failures.validation.RoleNotAString,
                    reason: "PluginFactory object's 'role' property not be a string (or undefined).",
                    help: `PluginFactory 'role' property is a ${typeof pluginFactory.role} instead.`
                }
            }
            ...
            if (roles.has(pluginFactory.role)) {
                return {
                    ...
                    reason: `A plugin with the role '${pluginFactory.role}' has already been installed.`
        ...
        if (pluginFactory.requires) {
            if (!Array.isArray(pluginFactory.requires)) {
                return {
                    canInitialize: false,
                    pluginFactory,
                    failureReason: failures.validation.RequiresNotAnArray,
                    reason: "PluginFactory's 'requires' property is not an array (or undefined).",
                    help: `PluginFactory's 'requires' property is a ${typeof pluginFactory.requires} instead.`,
                }
            }
            ...
            if (unfulfilledRequirements.length > 0) {
                return {
                    canInitialize: false,
                    pluginFactory,
                    failureReason: failures.validation.UnfulfilledPluginDependencies,
        ...
        if (pluginFactory.requiresRoles) {
            if (!Array.isArray(pluginFactory.requiresRoles)) {
                return {
                    canInitialize: false,
                    pluginFactory,
                    ...
                    help: `PluginFactory's 'requiresRoles' property is a ${typeof pluginFactory.requiresRoles} instead.`
                }
            }
            ...
            if (unfulfilledRoleRequirements.length > 0) {
                return {
                    canInitialize: false,
                    pluginFactory,
                    failureReason: failures.validation.UnfulfilledRoleDependencies,
    ...
    function dependencies (pluginFactory) {
        var res = {};
    ...
    function initialize(pluginFactory) {
        const validation = isInitializable(pluginFactory);
        if (validation.canInitialize) {
            return unsafeInitialize(pluginFactory);
        } else {
            return Fail({
    ...
    function unsafeInitialize (pluginFactory) {
        // Apply static hooks.
        ...
        // Create instance.
        const instance = pluginFactory.init(context, dependencies(pluginFactory));

        if (typeof instance !== "object" || instance === null) {
            const type = typeof instance === "object" ? "null" : typeof instance;
            return Fail({
                failureReason: failures.PluginNotAnObject,
                message: `Plugin instance from '${pluginFactory.name}' must be an object. Init function returned \`${String(instance)}\`, a ${type}, instead.`
            });
        }
        ...
        instanceHooks.forEach(function (hook, fn) {
            if (instance.hasOwnProperty(hook)) {
                fn(pluginFactory.name, instance[hook]);
            }
        });
        ...
        plugins.set(pluginFactory.name, instance);
        if (pluginFactory.role) {
            roles.set(pluginFactory.role, instance);
        }
    ...
    function pluginExportsOf (name) {
        if (plugins.has(name)) {
            ...
            return Fail({
                failureReason: failures.PluginNotInstalled,
                name
            });
        }
    }
    ...
    function roleExportsOf (name) {
        if (roles.has(name)) {
            ...
            return Fail({
                failureReason: failures.RoleNotInstalled,
                name
            });
        }
