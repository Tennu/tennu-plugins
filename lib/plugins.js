...
const PluginSystem = function (context) {
    ...
    const addInstanceHook = function addInstanceHook (hook, fn) {
    ...
    addInstanceHook.failures = failures.addInstanceHook;

    // Fn(hook: String | Symbol, fn: Fn(any) -> void) -> void
    const addStaticHook = function addStaticHook (hook, fn) {
        const failures = addStaticHook.failures;
        ...
        return setStaticHookResult.mapFail(function (failure) {
            return {
                failureType: failures.StaticHookAlreadyExists,
    ...
    addStaticHook.failures = failures.addStaticHook;
    ...
    // canInstallWith(pluginFactory: PluginFactory, assumedPlugins: [PluginFactory]) -> CannotInstallReason
    ...
    function unsafeInstall (pluginFactory) {
        ...
        if (typeof instance !== "object" || instance === null) {
            const type = typeof instance === "object" ? "null" : `a(n) ${typeof instance}`;
            return Fail({
                failureType: failures.PluginNotAnObject,
                message: `Plugin instance from '${pluginFactory.name}' must be an object. Init function returned \`${String(instance)}\`, ${aType(instance)}, instead.`,
                pluginFactory
            });
        }
