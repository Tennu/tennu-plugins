...
module.exports = function PluginLoader (pluginSystem, fs, require, systemname) {
    ...
    function use (pluginNames, path) {
        ...
        return collectResultArray(pluginNames, function (name) {
            return locate(name, path);
        })
        .andThen(function (paths) {
            const Plugins = zip(paths, pluginNames).map(load);

            return collectResultArray(zip(paths, pluginNames), load);
        })
        .andThen(function (Plugins) {
            const toInstall = [];
            const pluginInToInstall = new Map(Plugins.map(function (Plugin) {
                return [Plugin, false];
            }));
            ...
            const find = function (name, type, alreadyTrying) {
                for (const Plugin of Plugins) {
                    ...
                    //              If it's not, we'll catch it later when we try to install that.
                ...
                return Fail({
                    ...
                    message: `Plugin with ${type} of '${name}' required but neither installed nor in to be installed list.`,
                    dependencyType: type,
                    dependencyName: name
                });
            }

            const tryAddToInstall = function (Plugin, alreadyTrying) {
                if (pluginInToInstall.get(Plugin)) {
                    return Ok();
                }

                const validation = pluginSystem.isInstallableWith(Plugin, toInstall);

                if (validation.canInstall) {
                    toInstall.push(Plugin);
                    pluginInToInstall.set(Plugin, true);
                    return Ok();
                }
                ...
                if (validation.failureReason === failures.validation.UnfulfilledPluginDependencies) {
                    ...
                    return collectResultArray(validation.unfulfilledRequirements, function (name) {
                        ...
                        .andThen(function (Plugin) {
                            return tryAddToInstall(Plugin, alreadyTryingWithSelf);
                        });
                    }).andThen(function (_array_of_undefined) {
                        return tryAddToInstall(Plugin, alreadyTrying);
                    });
                }
            ...
            for (const Plugin of Plugins) {
                const result = tryAddToInstall(Plugin, []);
        ...
        .andThen(function (Plugins) {
            return collectResultArray(Plugins, function (Plugin) {
                return pluginSystem.install(Plugin);
            });
        })
    ...
    function load (data) {
        ...
        // NOTE(Havvy): Non-objects / null values will be caught in isInstallable.
